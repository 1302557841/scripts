...
const variable_1 = require('querystring');
const $ = new Env();
const variable_2 = 10000; //超时时间(单位毫秒)
...
let variable_7 = ''; // 推送到个人QQ: http://127.0.0.1/send_private_msg  群：http://127.0.0.1/send_group_msg 
let variable_8 = ''; //访问密钥
let variable_9 = ''; // 如果GOBOT_URL设置 /send_private_msg 则需要填入 user_id=个人QQ 相反如果是 /send_group_msg 则需要填入 group_id=QQ群 
...
let variable_16 = ''; //例如:127.0.0.1(环境变量名:TG_PROXY_HOST)
let variable_17 = ''; //例如:1080(环境变量名:TG_PROXY_PORT)
let variable_18 = ''; //tg代理配置认证参数
//Telegram api自建的反向代理地址(不懂可忽略,telegram机器人通知推送功能中非必填),默认tg官方api(环境变量名:TG_API_HOST)
let variable_19 = 'api.telegram.org';
// =======================================钉钉机器人通知设置区域===========================================
    ...
    return new Promise((resolve) => {
        if (TG_BOT_TOKEN && TG_USER_ID) {
            ...
            if (TG_PROXY_HOST && TG_PROXY_PORT) {
                const variable_29 = require('tunnel');
                const variable_30 = {
...
function ddBotNotify(text, desp) {
    return new Promise((resolve) => {
        ...
        if (DD_BOT_TOKEN && DD_BOT_SECRET) {
            const variable_31 = require('crypto');
            const variable_32 = Date.now();
            const variable_33 = crypto.createHmac('sha256', DD_BOT_SECRET);
            hmac.update(`${dateNow}\n${DD_BOT_SECRET}`);
            const variable_34 = encodeURIComponent(hmac.digest('base64'));
...
function ChangeUserId(desp) {
    const variable_35 = QYWX_AM.split(',');
    if (QYWX_AM_AY[2]) {
        const variable_36 = QYWX_AM_AY[2].split('|');
        let variable_37 = '';
        for (let variable_38 = 0; i < userIdTmp.length; i++) {
            const variable_39 = '账号' + (i + 1);
            const variable_40 = '签到号 ' + (i + 1);
    ...
    return new Promise((resolve) => {
        if (QYWX_AM) {
            ...
            $.post(options_accesstoken, (err, resp, data) => {
                html = desp.replace(/\n/g, '<br/>');
                var variable_42 = JSON.parse(data);
...
function iGotNotify(text, desp, params = {}) {
    return new Promise((resolve) => {
        if (IGOT_PUSH_KEY) {
            ...
            const variable_43 = new RegExp('^[a-zA-Z0-9]{24}$');
    ...
    return new class {
        ...
        runScript(t, s) {
            return new Promise(e => {
                let variable_38 = this.getdata("@chavy_boxjs_userCfgs.httpapi");
                i = i ? i.replace(/\n/g, "").trim() : i;
                let variable_46 = this.getdata("@chavy_boxjs_userCfgs.httpapi_timeout");
        ...
        loaddata() {
            if (!this.isNode()) return {};
            {
                ...
                const variable_47 = this.path.resolve(this.dataFile), s = this.path.resolve(process.cwd(), this.dataFile),
                    e = this.fs.existsSync(t), i = !e && this.fs.existsSync(s);
                if (!e && !i) return {};
                {
                    const variable_38 = e ? t : s;
                    try {
        ...
        lodash_get(t, s, e) {
            const variable_38 = s.replace(/\[(\d+)\]/g, ".$1").split(".");
            let variable_46 = t;
            for (const variable_47 of i) if (o = Object(o)[t], void 0 === o) return e;
            return o
        }
        ...
        getdata(t) {
            let variable_48 = this.getval(t);
            if (/^@/.test(t)) {
                const [, e, i] = /^@(.*?)\.(.*?)$/.exec(t), o = e ? this.getval(e) : "";
                if (o) try {
                    const variable_47 = JSON.parse(o);
        ...
        setdata(t, s) {
            ...
            if (/^@/.test(s)) {
                const [, i, o] = /^@(.*?)\.(.*?)$/.exec(s), h = this.getval(i),
                    a = i ? "null" === h ? null : h || "{}" : "{}";
                try {
                    const variable_48 = JSON.parse(a);
        ...
        get(t, s = (() => {
            ...
            t.headers && (delete t.headers["Content-Type"], delete t.headers["Content-Length"]), this.isSurge() || this.isLoon() ? $httpClient.get(t, (t, e, i) => {
                ...
                try {
                    const variable_49 = t.headers["set-cookie"].map(this.cktough.Cookie.parse).toString();
        ...
        time(t) {
            ...
            for (let variable_49 in s) new RegExp("(" + e + ")").test(t) && (t = t.replace(RegExp.$1, 1 == RegExp.$1.length ? s[e] : ("00" + s[e]).substr(("" + s[e]).length)));
            return t
        }

        msg(s = t, e = "", i = "", o) {
            const variable_50 = t => !t || !this.isLoon() && this.isSurge() ? t : "string" == typeof t ? this.isLoon() ? t : this.isQuanX() ? { "open-url": t } : void 0 : "object" == typeof t && (t["open-url"] || t["media-url"]) ? this.isLoon() ? t["open-url"] : this.isQuanX() ? t : void 0 : void 0;
        ...
        logErr(t, s) {
            const variable_49 = !this.isSurge() && !this.isQuanX() && !this.isLoon();
        ...
        wait(t) {
            ...
            const variable_48 = (new Date).getTime(), e = (s - this.startTime) / 1e3;
